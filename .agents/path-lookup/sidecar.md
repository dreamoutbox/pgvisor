...
- `crates/pgvisor-sidecar/src/main.rs` = Sidecar service entry point, WAL `archive` / `restore` CLI subcommands, container PID 1 signal listener (`SIGTERM`, `SIGINT`, `SIGQUIT`), Axum HTTP control server (`/control/status`, `/control/restore`, `/control/resync`, `/control/promote`, `/control/fence`, `/control/repoint`), auto-failover election monitor, and split-brain active fencing
